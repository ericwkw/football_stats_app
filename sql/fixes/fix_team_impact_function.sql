-- Fix for get_player_all_teams_impact function
-- This resolves the "team_id is ambiguous" error by fully qualifying all column references
...
BEGIN
  ...
  team_performance AS (
    ...
    SELECT
      tm.team_id,
      tm.player_participated,
      COUNT(*) AS matches,
      COUNT(*) FILTER (WHERE tm.result = 'win') AS wins,
      COUNT(*) FILTER (WHERE tm.result = 'draw') AS draws,
      COUNT(*) FILTER (WHERE tm.result = 'loss') AS losses,
      CASE 
        WHEN COUNT(*) > 0 THEN
          ROUND((COUNT(*) FILTER (WHERE tm.result = 'win')::numeric / COUNT(*)) * 100, 1)
        ELSE 0
      END AS win_rate
    FROM
      team_matches tm
    GROUP BY
      tm.team_id, tm.player_participated
  )
  SELECT
    t.id AS team_id,
    t.name AS team_name,
