-- Update the get_internal_all_player_statistics function to include clean_sheet_percentage
...
AS $$
  SELECT 
    ...
    CASE 
      WHEN COUNT(DISTINCT pms.match_id) > 0 AND p.position = 'Goalkeeper' 
      THEN ROUND((COALESCE(SUM(
        CASE 
          ...
          ELSE 0
        END
      ), 0) * 100.0) / COUNT(DISTINCT pms.match_id))
      ELSE NULL
